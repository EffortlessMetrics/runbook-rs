...
use runbook_hook_state::{transition as hook_transition, HookTransition};
...
fn reduce_hook(
    ...
    let transition = hook_transition(&hook, matcher.as_deref());
    match transition {
        HookTransition::Set(next_state) => {
            let session = state.ensure_session(&sid);
            session.agent_state = next_state;
        }
        HookTransition::EndSession => state.remove_session(&sid),
        HookTransition::Noop => {
            // No state change.
        }
    }
