...
use runbook_protocol::{
    AdjustmentKind, AgentState, ArmStyle, DialpadButton, HooksMode, PageDirection,
                ...
                PageDirection::Next => {
                    state.page =
                        state
                            .page
                            .checked_add(1)
                            .map_or(0, |next| if next == count { 0 } else { next });
                }
                PageDirection::Prev => {
                    state.page = state
                        .page
...
fn reduce_hook(
    ...
    match hook.as_str() {
        ...
        "RunbookPolicy" => {
            if let Some("blocked") = matcher.as_deref() {
...
"#;
    ...
    fn arm_and_dispatch() {
        ...
        assert!(matches!(effects.first(), Some(SideEffect::BroadcastRender)));
