...
`runbook-rs` has three verification surfaces:
...
Codecov is execution-surface telemetry. It does not prove operator state correctness, Claude Code hook correctness, WebSocket protocol correctness, Logitech plugin correctness, VS Code extension correctness, policy completeness, or release readiness.
